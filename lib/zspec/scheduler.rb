module ZSpec
  class Scheduler
    ...
    def store_example_ids(message, ids)
      @sink.hset(@examples_hash_name, sha(message), ids.to_json) unless message.include?("[")
    end

    def expand_example(message)
      examples = JSON.parse(@sink.hget(@examples_hash_name, sha(message)) || "[]")
      return [message] if examples.empty? || examples.length < 16
      examples.each_slice(examples.length / 16).map do |ids|
        message + "[" + ids.join(",") + "]"
      end
    end

    def sha(message)
      Digest::SHA1.file(message).hexdigest
    end
